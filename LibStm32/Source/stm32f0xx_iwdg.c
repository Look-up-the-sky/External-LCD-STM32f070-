...
void IWDG_Init(void)
{
	...
	IWDG_WriteAccessCmd(IWDG_WriteAccess_Enable);//解除寄存器保护 0x5555
	IWDG_SetPrescaler(IWDG_Prescaler_256);	//预分频
	IWDG_SetReload(0x271);		//设置重装载 0x271*256/40k = 4s
	IWDG_ReloadCounter();		//避免复位 0xAAAA
